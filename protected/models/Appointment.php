...
class Appointment extends CActiveRecord
{
    ...
    public function get_doctor_queue()
    {
        ...
        $sql = "SELECT @rownum:=@rownum+1 id,app_id,patient_id,doctor_id,patient_name,visit_id,
                    display_id,appointment_date,title,status
                    from(select app_id,patient_id,user_id doctor_id,patient_name,visit_id,display_id,appointment_date,title,status
    ...
    public function get_patient_queue()
    {
        if (isset($_GET['Appointment'])) {
            ...
            /*$cond = " and visit_id not in (select visit_id from transaction_log)
                        and visit_id in (SELECT visit_id FROM v_bloodtest_payment)";*/
            //$cond="";
