...
/**
 * This is the model class for table "ipd_tbl_ProgressNote".
 *
 * The followings are the available columns in table 'ipd_tbl_ProgressNote':
 * @property integer $id
 ...
 */
class ProgressNote extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('admit_id, progress, treatment, prepare_by, evt_date', 'required'),
			array('admit_id, prepare_by', 'numerical', 'integerOnly'=>true),
			array('progress, treatment', 'length', 'max'=>100),
			array('remarks', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, admit_id, progress, treatment, remarks, prepare_by, evt_date', 'safe', 'on'=>'search'),
		);
	}
	...
	public function getProgressNote()
	{
		...
		$sql="SELECT t1.id,admit_id,t2.patient_id,progress,treatment,remarks,
			(SELECT doctor_name FROM vw_user_info t4 WHERE t1.prepare_by=t4.userid) prepare_by,evt_date
					FROM ipd_tbl_ProgressNote t1
	 ...
	 * @return ProgressNote the static model class
	 */
